...
region$X2022.Production.Thousand.B.D. <- as.numeric(region$X2022.Production.Thousand.B.D.)

region <- region %>% mutate(Region = fct_reorder(Region, X2022.Production.Thousand.B.D., .desc = TRUE))

ggplot(region, aes(y=X2022.Production.Thousand.B.D., x=Region, fill=Region))+
  geom_col()+ 
  ...
  labs(title = "Oil by Region",
    ...
    theme_classic() +
    geom_text(aes(label = X2022.Production.Thousand.B.D.), vjust = -0.3, size = 3.5, color = "black") 
